...
select * from (values
  ('all', 10, '🗺️', 'Reading your map & pins',
    ...
    'Each dot is a door; its color is the type (IQ, Inspection Lead, Appointment, No-sit…). The legend on the right lists every type with a count. Gold stars are roofs we already installed (reference only). Tap a pin to see the homeowner.'),
  ...
  ('jr', 30, '▶', 'Start my day (required stops)',
  ...
  ('all', 40, '📅', 'Plan your day (around appointments)',
    ...
    'Tap 📅 Plan your day, enter your start time. It drops your appointments in at their times and fills the gaps with doors, keeping you close so you''re never late — all the way to 8 PM. Tap ✅ Appointment done to re-plan the rest off the real clock.'),
  ('sr', 45, '🧭', 'Enhanced Planned Day (manager-assigned)',
  ...
  ('all', 90, '⏳', 'Go-backs & come-backs',
    'After an inspection when the homeowner wants you back to go over it.',
    'Tap ⏳ Pending (come back), pick the day and an open time (only times you''re free show, so you never double-book). On that day it''s already a required stop on Start my day — you never keep a list.'),
  ('all', 100, '⭐', 'Referral',
