// netlify/functions/shovels-venice-scrub.js
//
// WORKER (HTTP-callable) for the Venice roof-age scrub via Shovels.ai. Netlify
// 403s HTTP calls to a *scheduled* function, so the schedule lives in the thin
// wrapper cron-shovels-venice-scrub.js and the actual work lives here where it
// can be fired by hand (GET ?dry=1 to preview counts, GET ?limit=N for a small
// batch, GET with no params for a full daily batch).
//
// Free-tier safe: checks up to 45 un-checked Venice INSPECTION-LEAD pins (~90
// Shovels credits) and hard-stops the moment credits_left drops below a floor.
//
...
// it checked so we never re-spend a credit on that house.
//
// Progress is tracked per-pin in extra.shovels_checked_at (no schema change), and a
// summary is written to app_settings.shovels_venice_scrub_log (+ a rolling daily tally).
//
