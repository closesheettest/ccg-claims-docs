...
//
// WEEKLY sign-off reminder: a manager signs off their department's whole week,
// once, after the week's work is done.
//
// Fires on FRIDAY, once that department's last shift of the day has ended (the
// office finishes at 5, the warehouse night crew at 11), so the ping lands when
// the week is actually over rather than at a fixed hour. If it's still unsigned,
// one follow-up goes out Monday morning; after that it stays quiet and the
// office can see what's outstanding on the Sign-off tab.
//
// At most one send per department per window. The stamp lives on the approvals
// row (status "open", totals.reminded_at / totals.reminded_mon) — no extra
// table, and signing off overwrites it with the real totals. Departments with
// nothing logged all week are skipped.
//
...
export const handler = async (event) => {
  ...
  const dow = new Date(`${now.date}T12:00:00Z`).getUTCDay();   // 0 Sun … 6 Sat
  const isFriday = dow === 5;
  const isMondayFollowUp = dow === 1 && nowMin >= 480 && nowMin <= 660;   // 8–11am ET
  if (!q.date && !force && !isFriday && !isMondayFollowUp) {
    return out(200, { ok: true, now, skipped_all: "not Friday evening or Monday morning — nothing to chase" });
  }
  const weekMonday = isMondayFollowUp ? addDays(weekStart(now.date), -7) : weekStart(now.date);
  const stampKey = isMondayFollowUp ? "reminded_mon" : "reminded_at";
  ...
  for (const d of depts) {
    ...
    // On Friday, wait until that department's last shift has actually ended.
    if (isFriday && !force && !q.date && !dayJustFinished(team, byShift, now, nowMin)) {
      skipped.push({ department: d.name, why: "still on shift — the week isn't over for them yet" });
      continue;
    }
    const day = q.date ? weekStart(q.date) : weekMonday;
    ...
    const [appr, entries] = await Promise.all([
      ...
      get(`payroll_time_entries?work_date=gte.${day}&work_date=lte.${weekEnd}&employee_id=in.(${team.map((t) => t.id).join(",")})&select=employee_id,checked_in_at,recap_at`),
    ]);
    if (appr[0]?.status === "approved") { skipped.push({ department: d.name, work_date: day, why: "already signed off" }); continue; }
    if (appr[0]?.totals?.[stampKey] && !force) { skipped.push({ department: d.name, week_start: day, why: "already reminded for this week" }); continue; }
    if (!entries.length && !force) { skipped.push({ department: d.name, week_start: day, why: `nothing logged in the week of ${day}` }); continue; }

    // Across the whole week, not one day.
    const byEmp = {};
    for (const e of entries) (byEmp[e.employee_id] = byEmp[e.employee_id] || []).push(e);
    ...
    const noShow = live.filter((e) => !(byEmp[e.id] || []).length).map(nameOf);
    const noRecap = live.filter((e) => (byEmp[e.id] || []).some((x) => x.checked_in_at && !x.recap_at)).map(nameOf);
    ...
    const range = `${pretty(day)}-${pretty(weekEnd)}`;
    const sms = `US Shingle: sign off ${d.name} for the week of ${range}.${gaps}\n\n${link.replace(/^https?:\/\//, "")}`;
    const html =
      `<p>Hi ${mgr.first_name},</p>` +
      `<p>The week of <b>${range}</b> is done. Payroll runs off what you approve, so please review <b>${d.name}</b>'s week and sign it — one sign-off covers the whole week.</p>` +
      (bits.length
      ...
      `<p style="color:#64748b;font-size:13px;">Open the Team tab → Week sign-off, and sign.</p>`;

    if (dry) {
      pinged.push({ department: d.name, week: range, manager: nameOf(mgr), phone: mgr.phone || null, email: mgr.email || null, no_checkin: noShow, no_recap: noRecap, not_onboarded: onboard, dry: true });
      continue;
    }
    // Stamp first, so a double invocation can't send twice.
    await upsertApproval(d.id, day, appr[0], stampKey);
    const sent = { sms: false, email: false };
    if (mgr.phone) sent.sms = await postOk("ghl-sms", { to: mgr.phone, name: nameOf(mgr), message: sms });
    if (mgr.email) sent.email = await postOk("send-email", { to: mgr.email, subject: `Sign off ${d.name} — week of ${range}`, html, fromName: MAIL_FROM_NAME });
    pinged.push({ department: d.name, week: range, manager: nameOf(mgr), no_checkin: noShow, no_recap: noRecap, not_onboarded: onboard, sent });
  }

  return out(200, { ok: true, week_start: weekMonday, when: isMondayFollowUp ? "monday follow-up" : "friday", dry, force, pinged, skipped });
};
...
async function upsertApproval(deptId, day, existing, stampKey) {
  await fetch(`${SB_URL}/rest/v1/payroll_week_approvals?on_conflict=department_id,week_start`, {
    ...
    body: JSON.stringify({
      ...
      totals: { ...(existing?.totals || {}), [stampKey]: new Date().toISOString() },
...
function weekStart(d) { const dow = new Date(`${d}T12:00:00Z`).getUTCDay(); return addDays(d, dow === 0 ? -6 : 1 - dow); }
function nowET() {
