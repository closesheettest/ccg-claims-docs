...
//   1. Flips the inspection to result = "retail", with a note.
...
//
// THE PA IS NOT FIRED BY A RETAIL SALE. If a PA is already assigned and working
// the claim ("Sit Sold PA" / waiting on docs), pa_id and pa_stage are LEFT ALONE:
// the homeowner bought the roof from us and still wants their adjuster to finish
// the claim. Only a deal with no PA on it leaves the PA pool.
//
// This used to null both fields on every retail flip, which — combined with the
// PA board reading result=eq.damage — silently detached a live adjuster and wiped
// the claim off their board mid-claim. pa-company-api now pulls retail deals that
// still have an assigned PA in an active stage, so one job is enough; there is no
// second "retail twin" job.
//
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // 1. Flip to retail + note. A PA already working the claim KEEPS it (see the
    //    header) — only a deal with no PA on it leaves the pool.
    ...
    log.push({
      ...
      text: paEngaged
        ? `🏠 ${bookedBy} went RETAIL at the door. The PA stays on it — the homeowner still wants the adjuster to finish the claim.`
    ...
    //    Runs whether or not a PA is on it: the job moves to the retail side, which
    //    is where the office and payroll expect a retail sale to live. The PA works
    //    the claim in the PA portal, not in JobNimbus, so nothing is stranded.
    ...
    return cors(200, JSON.stringify({ ok: true, task_id: taskId, pa_stays: paEngaged }));
  } catch (e) {
