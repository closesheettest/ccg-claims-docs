// netlify/functions/sync-pas-from-jn.js
//
// Pull the full JN user list and upsert each one as a Public Adjuster
// (PA) in our Supabase `pas` table. Same single-source-of-truth model
// as sync-inspectors-from-jn.js: JN owns the identity (name + email +
// JN user id); we add active flag + phone on our side.
//
// Upsert key: jn_user_id. New users insert with active=false (so they
// don't appear in the PA portal until the manager activates them).
// Existing rows have their name + email refreshed but NOT their active
// flag or phone — those are manager-controlled.
//
...
exports.handler = async (event) => {
  ...
  // 2. Pull existing PAs so we can decide insert vs update.
  const sbHeaders = {
    apikey: SB_KEY,
    Authorization: `Bearer ${SB_KEY}`,
    "Content-Type": "application/json",
  };
  const exRes = await fetch(`${SB_URL}/rest/v1/pas?select=id,jn_user_id,name,email`, { headers: sbHeaders });
  if (!exRes.ok) {
    return json(500, { ok: false, error: `Could not fetch PAs: ${await exRes.text()}` });
  }
  ...
  for (const u of users) {
    ...
    if (matched) {
      // Refresh name + email only. Don't touch active/phone.
      if (!dryRun) {
        const upd = await fetch(`${SB_URL}/rest/v1/pas?id=eq.${matched.id}`, {
          method: "PATCH",
          headers: sbHeaders,
      ...
      // Insert new — active=false so the PA portal doesn't include them
      // until the manager flips active.
      if (!dryRun) {
      ...
      inserted++;
      items.push({ jn_user_id: u.id, name, action: "inserted (inactive)" });
    }
  }
