...
const handler = async () => {
  ...
  // 2. Push each via the existing function. Serial with 500ms pace so
  //    JN's API isn't slammed and a single Lambda doesn't exceed
  //    Netlify's 26-second execution window. Cap per-run at 30 records
  //    (more than enough for a normal hour; anything left rolls into
  //    the next hour's run).
  ...
  for (let i = 0; i < todo.length; i++) {
    ...
    try {
      ...
      if (r.ok && d.ok && d.jn_updated) {
        // Stamp jn_pushed_at so subsequent runs skip this row.
        // (The manual UI handler stamps this too; in cron context we
        // do it directly here since we're not going through the React
        // adminPushResultToJn flow.)
        try {
          await fetch(
        ...
        // For retail records, also fire the retail swap (record_type
        // → Lead, location → retail, date_start → null).
        if (d.needs_retail_swap) {
          try {
            ...
            console.warn("retail swap fire-and-forget failed:", rec.id, e.message);
          }
        }
        okCount++;
      } else {
    ...
    if (i < todo.length - 1) await new Promise((r) => setTimeout(r, 500));
  }
