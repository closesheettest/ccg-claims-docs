...
exports.handler = async () => {
  ...
  try {
    ...
    out.job = {
      name: j.name || j.display_name, status_name: j.status_name,
      record_type_name: j.record_type_name, location: j.location,
      owners: (j.owners || []).map((o) => o.id), sales_rep: j.sales_rep,
      date_start: j.date_start, primary: j.primary && { id: j.primary.id, name: j.primary.name },
    };
    ...
    out.tasks = (td.results || td.tasks || td.data || []).map((t) => ({
      ...
      date_start: t.date_start, is_completed: t.is_completed, owners: (t.owners || []).map((o) => o.id),
    }));
