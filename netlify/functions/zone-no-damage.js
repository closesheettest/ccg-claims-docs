...
//
// "No damage" report for a regional manager. Pulls JobNimbus jobs
// whose status is any "No Damage…" (inspection found no roof damage), scoped to
// ONE zone, grouped
...
//
// GET /.netlify/functions/zone-no-damage?zone=Zone%204[&days=90]
...
// A status is "no damage" if, once stripped to letters/numbers/spaces,
// it starts with "no damage" — catches "No Damage", "No Damage - Cert
// Sent", "No-Damage", etc.
function matchesStatus(statusName) {
  const s = String(statusName || "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
  return s.startsWith("no damage");
}
